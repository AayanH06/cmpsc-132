...
def employee_update(d, bonus, year):
    ...
    previous_data = d[previous_year]

    d[year] = {}

    for employee_name in previous_data:
        old_details = previous_data[employee_name]

        new_salary = old_details[2] + bonus
        new_details = [old_details[0], old_details[1], new_salary]

        d[year][employee_name] = new_details
